...
public class PlyerControl : MonoBehaviour {
    ...
    {
        GameObject bulletObjet = Instantiate(bulletPrefab, transform.position + (transform.forward * 2), transform.rotation);
    }
    private Vector3 FindPositionMouse()
    {
    ...
    private void OrientatedPlayer()
    {
        Vector3 result = FindPositionMouse();
